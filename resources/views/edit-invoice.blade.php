...
@section('content')
            ...
            <form action="{{ route('edit-invoice', $detail->invoice) }}" method="post">
                    ...
                    <tbody>
                        @foreach ($products as $product)
                            <tr>
                                ...
                                <td><input type="text" name="price[]" value="{{ $product->price }}" readonly class="form-control"></td>
                                {{-- <td><input type="text" name="item_cost[]" value="{{ $product->item_cost }}" readonly class="form-control"></td> --}}
                                <td><input type="text" name="quantity[]" value="{{ $product->quantity }}" readonly class="form-control"></td>
                ...
                {{-- <table class="table">
                    <tr>
                        ...
                        <td>
                            <input type="text" name="attribute" class="form-control" placeholder="Attribute Name">
                        </td>
                        <td>
                ...
                {{-- <div class="customer_records my-2">
                    <div class="row">
                        <div class="col-md-4">
                            <input type="text" name="product_name" class="form-control" placeholder="Product Name">
                        </div>
    ...
    <script>
        $("#somebutton").click(function () {
            $("#container1").append("<tr><td class='p-3'><input type='text' name='product_name[]' class='form-control' placeholder='Product Name'></td><td class='p-3'><input type='text' name='attribute[]' class='form-control' placeholder='Attribute Name'></td><td class='p-3'><input type='text' name='price[]' placeholder='Price' oninput='this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');' class='form-control'></td><td class='p-3'><input type='text' name='quantity[]' placeholder='Quantity' oninput='this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');' class='form-control'></td> <td class='p-3'><button type='button' class='delete btn btn-primary' onclick='del()'> <i class='fa fa-trash'></i> </button></td></tr>"
            );
        });
    </script>
@endsection
